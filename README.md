...
**2026-09-18 修复进展：** 已通过 EOS 小规模受控实验，原 SFT/继续训练对照组均为 0/20 正常停止，修复组为 20/20；替换字符从 19/20 降至 0/20。BF16 保存重载复核也为 20/20 正常停止。聊天页面新增独立修复试验版，仍有错译，未重跑正式400题，不替代下面的旧正式结果。详见 `reports/REPAIR_PILOT_RESULTS.md`。
...
4. DPO 明显退化，GRPO 与 SFT 接近；仍需审查参考策略、adapter 加载、量化差异、奖励投机和数据质量，不能预设唯一原因。
